...
(function(exports) {
    var template = {
        ...
        formpanelTemplate: /*<MFFormPanelTemplate>*/'<div class="<%=options.classname.formpanel%>"><label class="formitem-label <%=options.classname.label%>" style="<%=options.style.label%>" <%=options.attr.label%>></label><%for(var i=0;i<options.buttons.length;i++){%><%if(options.buttons[i].submit){%><button class="<%=options.buttons[i].className%>" title="<%=options.buttons[i].title%>"><%=options.buttons[i].name%></button><%}else{%><button class="ml15 <%=options.buttons[i].className%>" title="<%=options.buttons[i].title%>"><%=options.buttons[i].name%></button><%}%><%}%></div>'/*</MFFormPanelTemplate>*/
    };
